...
- `SHIPS_SINCE_PERSONSIM: 0`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. The "graphics WINS" counter, repointed. (=0: RESET by iter-123 E4 realization-aware destiny — a person-sim epic.)
- `SMALL_SHIPS_SINCE_EPIC: 0`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC actually SHIPS. (=0: iter-123 shipped **EPIC E4** — realization-aware destiny. The iter-120/121 maturity-plateau "epic deferred" hold is OVER: E4 turned out NOT to need the owner's feel-check — the smallest viable version reframes waste at the perception layer, balance-neutral, fully sweep+biography-verifiable. The "decide→ship→veto-by-reaction" mandate held; the prior hold-spin was the dodge the arc forbids.)
...
- `ITERS_SINCE_DEPLOY: 4`       → **(owner 2026-06-14: "push to github.io after every 10 iteration") ≥10 ⇒ DEPLOY (run the banner recipe), then reset 0.** +1 per loop iteration. (=4: +iter-123; first deploy was iter-117 → next deploy ≈ iter 127.) Note: the public push needs a DIRECT owner go OR a `Bash(git push origin main)` allow-rule (the classifier blocks a feedback.md-only authorization).
