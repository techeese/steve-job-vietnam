...
- `SHIPS_SINCE_PERSONSIM: 2`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. The "graphics WINS" counter, repointed. (=2: Art epic + the iter-110 UI bug sweep shipped since the Mentor's Ledger soul-loop → person-sim is overdue; iter 111 MUST be person-sim.)
- `SMALL_SHIPS_SINCE_EPIC: 1`   → ≥3 ⇒ epic locks. (+1 iter 110 bug sweep.)
...
- `SHIPS_SINCE_DEPLOY: 1`       → **(LOCAL-DEV + BATCHED DEPLOY, owner 2026-06-14) ≥5 ⇒ this ship ALSO deploys to live (run the deploy recipe in the top banner), then reset 0.** +1 every ship (local commit on `mentors-ledger`). Between deploys: commit + verify locally, do NOT push. First deploy carries the whole current backlog. (=1: iter 110 bug sweep.)
...
- **NEXT LIVE PERSON-SIM EPIC = E4** (E1/E2a/E2b/E3 shipped as the Mentor's Ledger), then E7–E10 (the owner-steered "school you shape" texture family) + E5/E6. All person-sim-serving; the EPIC turn dequeues the top, respecting EPICS_SINCE_STRUCTURE (next epic must be STRUCTURE = `sim/person.js`).
