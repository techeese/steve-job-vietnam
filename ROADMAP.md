...
- `SMALL_SHIPS_SINCE_EPIC: 3`   → **≥3 ⇒ EPIC OWED — next firing is HARD-LOCKED to the EPIC track (polish ILLEGAL).** (=3: iter-142 cohort-summary + iter-143 share-card-face + iter-144 prized-wasted flavor. Last epic = iter-140 E7p. **NEXT FIRING: do an epic.** Best buildable option (EPICS_SINCE_STRUCTURE=0 → feature OR structure both legal): the **ui.js epilogue-cluster STRUCTURE extraction** (the real owed refactor, and the epilogue IS verifiable byte-identical via lives.sh `_essayText` — unlike the rAF render cluster) — extract essayDraft/buildCast/shareCard + helpers into a screen module via a shared-UI-context. Fallbacks: E-UNDERDOG ckpt2 (player-agency wildcard-admit toggle) or grain-flavored teachers (balance — owner-playtest). Lean: the ui.js epilogue extraction (real structure, verifiable, breaks the marginal-polish run).)
- `EPICS_SINCE_STRUCTURE: 0`    → ≥2 ⇒ next epic must be STRUCTURE. (=0: **REVIEW-ESCAPE iter 141 (skill's option b) — RESET with a logged verdict.** RIGOROUS review: ui.js is 1824 ln, ONE IIFE, ~229 closure-helper uses (el/esc/openModal/P) = dense bidirectional coupling; the clean config/copy leaf is already taken (iter-134); the render cluster is rAF-driven so a render extraction CAN'T be byte-identical-verified headlessly. The only remaining extraction = the **shared-UI-context refactor** — high-effort, partially-unverifiable, and LOW-immediate-value (the UI features it'd enable, E11 walk-in rooms / E12-14 identity, are owner-gated → no near-term need). Verdict: **no clean+safe+verifiable+worthwhile extraction available now.** QUEUED: do the shared-context refactor (extract the epilogue/essay cluster first — it IS verifiable via lives.sh `_essayText`) WHEN a UI feature needs it or ui.js becomes painful. Counter reset; feature-epics legal again. Prior structure: iter-134 data/content split, iter-127 person.js carve.)
...
- `ITERS_SINCE_DEPLOY: 5`       → **(owner 2026-06-14: "push to github.io after every 10 iteration") ≥10 ⇒ DEPLOY (run the banner recipe), then reset 0.** +1 per loop iteration. (=5: +iter-144. **DEPLOYED iter 139 — LIVE & VERIFIED** `main 40916f7..559adaf` pushed; live serves `?v=1781467721271`. 10 commits now public: match-legibility · mood+burnout+warning · protégé arc/journal/capstone · E8 faculty · tuition trade-off · content.js split · mood-emote. Pre-deploy gate GREEN, bot BOTOK, 5 sweep ✓ / 0 breakage; standing authorization held. Next deploy ≈ iter 149.)
