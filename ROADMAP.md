...
- `SMALL_SHIPS_SINCE_EPIC: 0`   → **≥3 ⇒ EPIC owed; the next firing HARD-LOCKS to the EPIC track.** Reset 0 only when an EPIC SHIPS. (=0: **EPIC SHIPPED iter-172 [EVOLUTION] — Three Theses, Three Epilogues** (closed the §D-3 craft-vs-balanced epilogue gap; reset the run of small beats 168–171). iter-173 was an incident-response HOTFIX to that epic (debt-valve, exempt) — does not increment. The 168→171 run HAD tripped this to 4 [the documented "tiny additive features" failure mode]; the iter-172 epic legitimately cleared it.)
