...
- `SMALL_SHIPS_SINCE_EPIC: 3`   → **≥3 ⇒ EPIC owed; the next firing HARD-LOCKS to the EPIC track.** Reset 0 only when an EPIC SHIPS. (=3: iter-194 diamond-waste symmetry + iter-193 cohortBeat emergent-narrative + iter-184 lab-livestream income (all person-sim/economy polish). **⚠ NEXT firing IS NOW HARD-LOCKED to the EPIC track — polish is illegal. Legal moves: advance an in-flight epic, or dequeue+plan+start the top Epic backlog item. Most person-sim epics are owner-gated (E6/E7-majors/E10) or graphics (E11, illegal this arc); the non-gated candidate = E8 ckpt2 grain-flavored teacher trade-offs (a real balance change, sweep-gated) — OR the owed ui.js STRUCTURE refactor if a clean+verifiable extraction is found.** Epic that last reset this: **iter-180-182 owner LIVING-ECONOMY epic** (ckpts 1-3). iter-183 structure was the owed paydown.)
