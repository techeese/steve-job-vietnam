...
- `SHIPS_SINCE_PERSONSIM: 4`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. The "graphics WINS" counter, repointed. (=4: Art epic + iter-110 bugs + iter-112 stroll + iter-113 de-sync — all owner-asked campus preemptions; person-sim is BADLY OVERDUE. Next firing is EPIC-locked → the `sim/person.js` STRUCTURE epic, which IS people-dev infra.)
- `SMALL_SHIPS_SINCE_EPIC: 3`   → **≥3 ⇒ EPIC-LOCKED: the next firing MUST be the EPIC track (polish is ILLEGAL).** (+1 iter 110 bugs, +1 iter 112 stroll, +1 iter 113 de-sync.) Combined with EPICS_SINCE_STRUCTURE=2 → next epic = STRUCTURE = `sim/person.js`.
...
- `SHIPS_SINCE_DEPLOY: 3`       → **(LOCAL-DEV + BATCHED DEPLOY, owner 2026-06-14) ≥5 ⇒ this ship ALSO deploys to live (run the deploy recipe in the top banner), then reset 0.** +1 every ship (local commit on `mentors-ledger`). Between deploys: commit + verify locally, do NOT push. First deploy carries the whole current backlog. (=3: iter 110 bugs + iter 112 stroll + iter 113 de-sync; 2 more ships to first live deploy.)
