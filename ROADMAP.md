...
- `SMALL_SHIPS_SINCE_EPIC: 1`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC SHIPS. (=1: iter-135 follow-journal polish. Last epic = iter-134 STRUCTURE.)
...
- `ITERS_SINCE_DEPLOY: 6`       → **(owner 2026-06-14: "push to github.io after every 10 iteration") ≥10 ⇒ DEPLOY (run the banner recipe), then reset 0.** +1 per loop iteration. (=6: +iter-135. Last deploy iter 129 — LIVE & VERIFIED `main d8cc425..40916f7` pushed; live serves `?v=1781463105023` (the iter-118→128 backlog now public: E4/E4.1/protégé-arc/E5/STRUCTURE/E-UNDERDOG). Pre-deploy gate GREEN, bot BOTOK, 5 sweep ✓ / 0 breakage; standing direct authorization held. Next deploy ≈ iter 139.)
