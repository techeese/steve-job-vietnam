...
- `SHIPS_SINCE_PERSONSIM: 3`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ HARD-LOCKS to a person-sim pick/epic.** Reset 0 only on a person-sim ship. (=3: iters 180-184 were economy/structure (non-person-sim) — owner's explicit economy steer PREEMPTED the lock (like iter-158). The steer's 3 ckpts + the lab extension are now delivered. **⚠ WATCH: 5 non-soul ships in a row = economy over-indexing. The owner's #1 value is the SOUL ("money is the body, not the soul"). Next firing should lean BACK toward soul/§D OR await a fresh owner steer — NOT a 6th economy/building ship.** Prior person-sim: iter-179 cohortBeat.)
- `SMALL_SHIPS_SINCE_EPIC: 1`   → **≥3 ⇒ EPIC owed; the next firing HARD-LOCKS to the EPIC track.** Reset 0 only when an EPIC SHIPS. (=1: iter-184 lab-livestream income (small economy extension of the owner's "or similar building", after the epic). Epic that reset this: **iter-180-182 owner LIVING-ECONOMY epic** (ckpts 1-3). iter-183 structure was the owed paydown.)
