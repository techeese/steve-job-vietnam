...
- `SMALL_SHIPS_SINCE_EPIC: 2`   → ≥3 ⇒ epic track locks (→ resolve via review-defer; epic AWAITS OWNER STEER, per iter-92). (102 PWA + 103 save-bugfix = 2.) **Plateau mode** validated: a robustness bug-hunt (iter 103) caught a real save-compat bug — so plateau firings are best spent on active probing/maintenance (which finds genuine issues) over additive micro-churn. Remaining feature value awaits steer.
