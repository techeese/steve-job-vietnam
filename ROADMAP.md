...
- `SHIPS_SINCE_PERSONSIM: 0`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. (=0: iter-151 per-kid peak-suppression warning on the inspect card — legibility-of-transformation, Compass 1/2.)
- `SMALL_SHIPS_SINCE_EPIC: 2`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC SHIPS. (=2: iter-150 protégé-coda + iter-151 peak-warning polish. Last epic = iter-149 STRUCTURE epilogue.js. **iter 152 is the last legal polish before the lock; 153 epic-owed. 50-FIRING REVIEW (iter 150): safe person-sim space is MATURE; small ships are correct (highest-value-safe on a release-ready build); real unlock = owner re-engagement, surfaced not forced.**)
