...
- `SMALL_SHIPS_SINCE_EPIC: 3`   → **≥3 — epic track locked → resolve via review-defer (epic AWAITS OWNER STEER, per iter-92).** (102 PWA + 103 save-maps-fix + 104 favId-fix = 3.) **Plateau mode**: the robustness vein (103–104) hardened save-compat to provably LOSSLESS + GATE_SAVE-guarded — that vein is now closed too. With features/content/coverage/distribution/robustness all done, genuine safe value is essentially exhausted; lean to periodic maintenance + awaiting steer.
