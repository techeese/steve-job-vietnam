...
- `SMALL_SHIPS_SINCE_EPIC: 2`   → **≥3 ⇒ EPIC owed (next firing's polish is illegal — must be EPIC track; and EPICS_SINCE_STRUCTURE=2 ⇒ that epic MUST be STRUCTURE).** Reset 0 only when an EPIC SHIPS. (=2: iter-132 mood-warning + iter-133 protégé-capstone, both polish. Last epic = iter-131 mood. **Heads-up: the next firing that hits ≥3 forces the ui.js shared-UI-context refactor OR a rigorous review-escape.**)
...
- `ITERS_SINCE_DEPLOY: 4`       → **(owner 2026-06-14: "push to github.io after every 10 iteration") ≥10 ⇒ DEPLOY (run the banner recipe), then reset 0.** +1 per loop iteration. (=4: +iter-133. Last deploy iter 129 — LIVE & VERIFIED `main d8cc425..40916f7` pushed; live serves `?v=1781463105023` (the iter-118→128 backlog now public: E4/E4.1/protégé-arc/E5/STRUCTURE/E-UNDERDOG). Pre-deploy gate GREEN, bot BOTOK, 5 sweep ✓ / 0 breakage; standing direct authorization held. Next deploy ≈ iter 139.)
