...
- `SHIPS_SINCE_PERSONSIM: 0`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. The "graphics WINS" counter, repointed. (=0: iter-125 protégé in-school arc — a person-sim ship.)
- `SMALL_SHIPS_SINCE_EPIC: 2`   → **≥3 ⇒ EPIC owed (next firing's polish is illegal — must be EPIC track).** Reset 0 only when an EPIC actually SHIPS. (=2: iter-124 E4.1 + iter-125 protégé-arc, both person-sim polish. Last epic = iter-123 E4. **The next EPIC** = the STRUCTURE move (alumni FSM → `js/sim/person.js`, EPICS_SINCE_STRUCTURE=1) OR the ⭐ E-UNDERDOG admissions epic — but a feature-epic bumps EPICS_SINCE_STRUCTURE to 2, forcing structure after. Lean: do the STRUCTURE carve next, then E-UNDERDOG.)
- `EPICS_SINCE_STRUCTURE: 1`    → ≥2 ⇒ next epic must be STRUCTURE. (=1: iter-123 E4 was a FEATURE-epic. **Next epic must be STRUCTURE** — queued: the alumni FSM (`makeAlumnus`/`transition`/`cascadeOutcome`) → `js/sim/person.js`, behavior byte-identical, de-risks E5/E6. Prior structure move: iter-114 `sim/person.js` carve.)
...
- **★ PEOPLE-FIRST ARC ACTIVE (2026-06-14, owner course-correction).** The game's SOUL = the deep simulation of a person becoming themselves (talent × education → realized/wasted/distorted). Plateau rule SUSPENDED: the person-sim (E4–E6 below) IS the epic supply; "awaiting steer" is illegal as a reason to ship polish. Graphics/distribution/audio = production-broken-only. Arc releases only on a MEASURED gate + EXPLICIT owner confirm (see SKILL.md PEOPLE-FIRST MANDATE + VISION.md). **E1+E2+E3+E4 SHIPPED — the soul-loop is now: grain↔preset `CONFIG.MATCH` coupling + scarce mentor attention + epilogue NAMES the waste (Mentor's Ledger) + iter-123 realization-aware destiny (craft's QUIET waste — the prodigy who settled — now named, closing the [EVOLUTION] §C-2 gap). Next person-sim epic = E5 (discoverable talent) or the campus-life E7/E11 family — BUT the next EPIC must be STRUCTURE (alumni FSM → person.js, EPICS_SINCE_STRUCTURE=1). Arc NOT yet released — needs owner confirm on the MEASURED gate.**
- `ITERS_SINCE_DEPLOY: 6`       → **(owner 2026-06-14: "push to github.io after every 10 iteration") ≥10 ⇒ DEPLOY (run the banner recipe), then reset 0.** +1 per loop iteration. (=6: +iter-125; first deploy was iter-117 → next deploy ≈ iter 127.) Note: the public push needs a DIRECT owner go OR a `Bash(git push origin main)` allow-rule (the classifier blocks a feedback.md-only authorization).
- `LAST_EPIC: iter 123 FEATURE — E4 realization-aware destiny (FLOURISH/realFrac/realClass; a.fs.real carried gap; epilogue names craft's QUIET waste "đáng lẽ đã có thể hơn thế" + the loud waste + the lift; uSettle sweep sensor). Balance-neutral (destiny/economy/🍎/arrests unchanged); resolves the [EVOLUTION] §C-2 craft-fails-nobody gap at the perception layer. Prior: iter-114 STRUCTURE person.js carve, Mentor's Ledger soul-loop, Art & Polish.`
...
- **⭐ E-UNDERDOG — "Take a chance on a modest kid" (the missing upward-mobility arc).** *(person-sim; surfaced iter-124 by probe; owner-in-the-loop balance epic.)* **The finding:** admissions admits high-score→high-seed only, so the cohort has ~no seed≤2 students — the most inspiring education narrative (an underdog the school believed in, who rises) is **structurally impossible**, and it's the SAME upstream skew the craft-symmetry saga kept hitting. **The epic:** a SCARCE, player-controlled "wildcard admit" lever (e.g. 1/year — take one below-cutoff kid you see something in), bounded so it can't flood the cohort or break the economy (tuition×SV). Pairs with mentoring: admit + mentor a modest kid → they can realize a good life → the "vượt xa điểm xuất phát" lift becomes REACHABLE (re-add it then). **Why owner-in-the-loop:** it's a real balance change touching admissions + economy + the realize/waste spread — wants a sweep-gate (no dominance, spread holds, 🍎 stays rare) AND owner playtest of the resulting feel. NOT deferred-as-dodge — evidence-backed + scoped. OWNER: confirm the wildcard-admit direction or steer.
