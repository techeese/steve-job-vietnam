...
- `SHIPS_SINCE_PERSONSIM: 1`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. (=1: iter-134 was a GENERAL structure move (data/content split — not person-sim-domain, unlike the iter-127 person.js carve) → increments. **Next firing HARD-LOCKED to person-sim.** Prior person-sim ship: iter-133 protégé-capstone.)
- `SMALL_SHIPS_SINCE_EPIC: 0`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC SHIPS. (=0: iter-134 shipped the STRUCTURE epic (data/content split). Prior polish: iter-132 mood-warning, iter-133 protégé-capstone.)
- `EPICS_SINCE_STRUCTURE: 0`    → ≥2 ⇒ next epic must be STRUCTURE. (=0: **RESET by iter-134 STRUCTURE** — CONTENT (414 ln copy) carved data.js→`js/content.js`, byte-identical (gate green, sweep 5✓, bot BOTOK identical metrics). Took the CLEAN config/copy leaf instead of review-escaping the high-coupling ui.js. Feature-epics legal again. **Next structure candidate: ui.js (1807 ln) → the shared-UI-context refactor — high-coupling, queued for when a UI feature (E11/E12-14, owner-gated) needs it; review-escape if it comes due while still non-urgent.** Prior: iter-127 person.js carve.)
