...
- `SMALL_SHIPS_SINCE_EPIC: 3`   → **≥3 — next firing is HARD-LOCKED to the EPIC track, which AWAITS OWNER STEER.** No worthwhile autonomous epic exists (feature epics taste-blocked; the ui split deferred per iter-92 review) → resolve the slot via a brief review-that-defers and keep shipping plateau-mode safe value, until steer. (93 ticker-idle + 94 hidden-ticker + 95 giftVt-bugfix = 3 ships.)
