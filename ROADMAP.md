...
- `SMALL_SHIPS_SINCE_EPIC: 1`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC SHIPS. (=1: iter-137 tuition trade-off (balance fix). Last epic = iter-136 E8.)
...
- `ITERS_SINCE_DEPLOY: 8`       → **(owner 2026-06-14: "push to github.io after every 10 iteration") ≥10 ⇒ DEPLOY (run the banner recipe), then reset 0.** +1 per loop iteration. (=8: +iter-137. Last deploy iter 129 — LIVE & VERIFIED; **deploy due in ~2 firings (iter 139); attempt the push then per the standing authorization.** `main d8cc425..40916f7` pushed; live serves `?v=1781463105023` (the iter-118→128 backlog now public: E4/E4.1/protégé-arc/E5/STRUCTURE/E-UNDERDOG). Pre-deploy gate GREEN, bot BOTOK, 5 sweep ✓ / 0 breakage; standing direct authorization held. Next deploy ≈ iter 139.)
