...
- `SMALL_SHIPS_SINCE_EPIC: 0`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC actually SHIPS. (=0: iter-131 shipped **EPIC mood-as-live-dimension**. Prior: iter-130 match-legibility polish.)
- `EPICS_SINCE_STRUCTURE: 2`    → **≥2 ⇒ the NEXT epic MUST be a STRUCTURE move.** (=2: iter-128 E-UNDERDOG + iter-131 mood, both FEATURE-epics. **Next epic = STRUCTURE.** Candidate: ui.js (1781 ln, the painful file) → screens.js — high-coupling (`el` ~200×, bidirectional), needs the shared-UI-context refactor; assess clean-leaf-ness, else review-escape. Prior structure: iter-127 person.js lifecycle carve.)
...
- `ITERS_SINCE_DEPLOY: 2`       → **(owner 2026-06-14: "push to github.io after every 10 iteration") ≥10 ⇒ DEPLOY (run the banner recipe), then reset 0.** +1 per loop iteration. (=2: +iter-131. Last deploy iter 129 — LIVE & VERIFIED `main d8cc425..40916f7` pushed; live serves `?v=1781463105023` (the iter-118→128 backlog now public: E4/E4.1/protégé-arc/E5/STRUCTURE/E-UNDERDOG). Pre-deploy gate GREEN, bot BOTOK, 5 sweep ✓ / 0 breakage; standing direct authorization held. Next deploy ≈ iter 139.)
