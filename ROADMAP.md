...
- `SMALL_SHIPS_SINCE_EPIC: 0`   → ≥3 ⇒ epic track locks. (Reset iter 96: the locked slot was resolved by deferring to the standing iter-92 architecture review — no worthwhile autonomous epic exists; feature epics await steer, the ui→screens.js split waits for strain/owner-OK.) The loop continues in **plateau mode**: ship genuine safe value (completeness/content/bugfix/juice/maintenance), resolve each epic-lock via review-defer, until owner steer arrives for the next true epic.
