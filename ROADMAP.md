...
- `SHIPS_SINCE_PERSONSIM: 1`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. (=1: iter-145 was a GENERAL structure move (uikit.js — not person-sim-domain) → increments. **Next firing HARD-LOCKED to person-sim.** Prior person-sim ships: iter-142 cohort-summary, 143 share-face, 144 prized-wasted.)
- `SMALL_SHIPS_SINCE_EPIC: 0`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC SHIPS. (=0: iter-145 shipped the STRUCTURE epic (uikit.js UI-toolkit extraction). Prior polish: iter-142/143/144.)
...
- `ITERS_SINCE_DEPLOY: 6`       → **(owner 2026-06-14: "push to github.io after every 10 iteration") ≥10 ⇒ DEPLOY (run the banner recipe), then reset 0.** +1 per loop iteration. (=6: +iter-145. **DEPLOYED iter 139 — LIVE & VERIFIED** `main 40916f7..559adaf` pushed; live serves `?v=1781467721271`. 10 commits now public: match-legibility · mood+burnout+warning · protégé arc/journal/capstone · E8 faculty · tuition trade-off · content.js split · mood-emote. Pre-deploy gate GREEN, bot BOTOK, 5 sweep ✓ / 0 breakage; standing authorization held. Next deploy ≈ iter 149.)
