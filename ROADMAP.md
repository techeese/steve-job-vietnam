...
- `SHIPS_SINCE_PERSONSIM: 0`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. (=0: iter-148 empty-🍎 cause prose (even/grind/mixed) — biography / legibility-of-the-peak-vs-median trade-off, Compass 1/2.)
- `SMALL_SHIPS_SINCE_EPIC: 3`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC SHIPS. (=3: iter-146 + iter-147 + iter-148 person-sim polish. Last epic = iter-145 STRUCTURE uikit.js. **iter 149 IS HARD-LOCKED TO THE EPIC TRACK — a polish pick is ILLEGAL. Legal moves: advance an in-flight epic, or dequeue+plan+start the top backlog item. Note iter 149 is ALSO deploy-due (ITERS_SINCE_DEPLOY→10) → ship the epic checkpoint AND deploy. Candidate epic: the queued STRUCTURE move (epilogue/essay cluster carve from ui.js, verifiable via lives.sh `_essayText`) — autonomous-safe + behavior-neutral, so it CANNOT destabilize the release-ready balance the owner must evaluate; OR an owner-unblocked feature epic if the owner returns.**)
