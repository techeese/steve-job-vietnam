...
- `SMALL_SHIPS_SINCE_EPIC: 1`   → ≥3 ⇒ the next firing is **HARD-LOCKED to the EPIC track**. Reset to 0 only when an epic SHIPS (or, per iter 92, the epic slot is resolved by a review-that-defers when no worthwhile autonomous epic exists — and the next true epic AWAITS OWNER STEER, see below). (iter 93 = POLISH: ticker idle-lines wired up + 9 new, plateau-mode safe value.)
