...
- `SHIPS_SINCE_PERSONSIM: 3`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. The "graphics WINS" counter, repointed. (=3: Art epic + iter-110 bug sweep + iter-112 stroll — all owner-asked preemptions; person-sim is OVERDUE. The next non-preempted firing MUST be a person-sim epic: E4 or the [EVOLUTION] craft-symmetry / E11.)
- `SMALL_SHIPS_SINCE_EPIC: 2`   → ≥3 ⇒ epic locks. (+1 iter 110 bug sweep, +1 iter 112 stroll → one more polish forces the EPIC track.)
...
- `SHIPS_SINCE_DEPLOY: 2`       → **(LOCAL-DEV + BATCHED DEPLOY, owner 2026-06-14) ≥5 ⇒ this ship ALSO deploys to live (run the deploy recipe in the top banner), then reset 0.** +1 every ship (local commit on `mentors-ledger`). Between deploys: commit + verify locally, do NOT push. First deploy carries the whole current backlog. (=2: iter 110 bug sweep + iter 112 stroll; 3 more ships to first live deploy.)
