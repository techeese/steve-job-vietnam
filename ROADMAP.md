...
- `SMALL_SHIPS_SINCE_EPIC: 1`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC actually SHIPS. (=1: iter-132 mood-risk-warning polish. Last epic = iter-131 mood.)
- `EPICS_SINCE_STRUCTURE: 2`    → **≥2 ⇒ the NEXT epic MUST be a STRUCTURE move (a feature-epic is illegal until structure lands).** (=2: iter-128 E-UNDERDOG + iter-131 mood. **ASSESSED iter-132:** ui.js (1781 ln) is one tightly-coupled IIFE — the ~430-ln render cluster + panels/modals all share closure state/canvas/helpers; NO clean leaf without the shared-UI-context refactor first (high-effort, zero immediate value — future UI features E11/E12-14 are owner-gated). So the structure debt = **the shared-UI-context refactor**, queued for when a UI feature actually needs it (or invoke the review-escape if the next epic comes due first + it's still non-urgent). iter-132 did a polish (no epic → debt not triggered). Prior structure: iter-127 person.js carve.)
