...
- `SHIPS_SINCE_PERSONSIM: 0`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. The "graphics WINS" counter, repointed. (=0: iter-126 E5 discoverable talent — a person-sim epic.)
...
- `EPICS_SINCE_STRUCTURE: 2`    → **≥2 ⇒ the NEXT epic MUST be a STRUCTURE move (a feature-epic is illegal until structure lands).** (=2: iter-123 E4 + iter-126 E5, both FEATURE-epics. **Next epic = STRUCTURE.** Candidate: the person destiny/lifecycle (makeAlumnus/cascade/realization/favBeat) → `js/sim/person.js`. NOTE: assess clean-leaf-ness first — these are SCATTERED across engine.js (lines ~45/310/607/761/1037) + school-coupled, and engine.js (1203 ln) isn't yet painful; if no clean+safe+worthwhile contiguous extraction, invoke the review-escape with that verdict + reset. Prior structure: iter-114 person.js carve.)
