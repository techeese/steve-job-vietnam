...
- `SHIPS_SINCE_PERSONSIM: 0`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. (=0: iter-147 🍎-keynote / 🚔-arrest ticker highlight — legibility-of-transformation, Compass 1/2.)
- `SMALL_SHIPS_SINCE_EPIC: 2`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC SHIPS. (=2: iter-146 + iter-147 soul-ticker-highlight polish. Last epic = iter-145 STRUCTURE uikit.js. **iter 148 reads 2 (< 3) → last legal polish; a polish there makes it 3 ⇒ iter 149 EPIC-LOCKED. Given the anti-timidity bias + 2 polish in a row, iter 148 SHOULD lean to the epic now rather than squeeze one more.**)
...
- `ITERS_SINCE_DEPLOY: 8`       → **(owner 2026-06-14: "push to github.io after every 10 iteration") ≥10 ⇒ DEPLOY (run the banner recipe), then reset 0.** +1 per loop iteration. (=8: +iter-147. **DEPLOYED iter 139 — LIVE & VERIFIED** `main 40916f7..559adaf` pushed; live serves `?v=1781467721271`. 10 commits now public: match-legibility · mood+burnout+warning · protégé arc/journal/capstone · E8 faculty · tuition trade-off · content.js split · mood-emote. Pre-deploy gate GREEN, bot BOTOK, 5 sweep ✓ / 0 breakage; standing authorization held. Next deploy ≈ iter 149.)
