...
- `SMALL_SHIPS_SINCE_EPIC: 3`   → **≥3 ⇒ EPIC owed; THIS COUNTER IS TRIPPED — the NEXT firing HARD-LOCKS to the EPIC track (a small/polish pick is ILLEGAL).** Reset 0 only when an EPIC SHIPS. (=3: iter-174 channeledMaker §C-4 beat + iter-176 climax de-dup + iter-179 cohortBeat (the broader cohort's during-play transformation — mark 5+2). Epic that reset this: **iter-172 [EVOLUTION] Three Theses, Three Epilogues**. iter-173/175/177 were exempt infrastructure/maintenance. **NEXT FIRING = EPIC.** Harvest the background frame-reset critic's gap (iter-175, running) as that epic if it has landed; else the legal epic moves are E11 (campus-life, owner-flagged) or the owed STRUCTURE (EPICS_SINCE_STRUCTURE=1 → a feature epic is still legal). Do NOT ship a 4th small beat.)
