...
- `SMALL_SHIPS_SINCE_EPIC: 0`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC SHIPS. (=0: iter-136 shipped **EPIC E8** (reputation-gated faculty). Prior polish: iter-135 follow-journal.)
...
- `ITERS_SINCE_DEPLOY: 7`       → **(owner 2026-06-14: "push to github.io after every 10 iteration") ≥10 ⇒ DEPLOY (run the banner recipe), then reset 0.** +1 per loop iteration. (=7: +iter-136. Last deploy iter 129 — LIVE & VERIFIED; **next deploy due ≈ iter 139.** `main d8cc425..40916f7` pushed; live serves `?v=1781463105023` (the iter-118→128 backlog now public: E4/E4.1/protégé-arc/E5/STRUCTURE/E-UNDERDOG). Pre-deploy gate GREEN, bot BOTOK, 5 sweep ✓ / 0 breakage; standing direct authorization held. Next deploy ≈ iter 139.)
