...
- `SHIPS_SINCE_PERSONSIM: 0`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. (=0: iter-155 FLOW — high mood → faster genuine growth (mood economy, Compass 1/2; VISION #3 burnout-vs-flow). A MECHANIC deepening, not surfacing.)
- `SMALL_SHIPS_SINCE_EPIC: 2`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC SHIPS. (=2: iter-154 grad-reading + iter-155 flow. Last epic = iter-153 E9. **iter 156 is the last legal polish; 157 epic-owed.**)
