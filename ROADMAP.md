...
- `SHIPS_SINCE_PERSONSIM: 0`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. The "graphics WINS" counter, repointed. (=0: iter-114 `sim/person.js` STRUCTURE carve shipped — people-dev infra, the arc's blessed person-sim ship.)
- `SMALL_SHIPS_SINCE_EPIC: 0`   → ≥3 ⇒ EPIC-LOCKED. (=0: the iter-114 epic shipped.)
...
- `SHIPS_SINCE_DEPLOY: 4`       → **(LOCAL-DEV + BATCHED DEPLOY, owner 2026-06-14) ≥5 ⇒ this ship ALSO deploys to live (run the deploy recipe in the top banner), then reset 0.** +1 every ship (local commit on `mentors-ledger`). Between deploys: commit + verify locally, do NOT push. First deploy carries the whole current backlog. (=4: +iter-114 carve; **the NEXT ship hits 5 → first live deploy** — carries everything since `main`.)
