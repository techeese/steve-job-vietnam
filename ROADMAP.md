...
- `SHIPS_SINCE_PERSONSIM: 1`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. (=1: iter-149 was a STRUCTURE refactor (epilogue.js) — did NOT touch the person-sim. **iter 150 IS HARD-LOCKED to a person-sim pick.**)
- `SMALL_SHIPS_SINCE_EPIC: 0`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC SHIPS. (=0: **EPIC SHIPPED iter 149 — STRUCTURE epilogue.js carve** (buildCast/cardLife/shareCard → js/epilogue.js; byte-identical essay md5 7b135beb, gate/bot/sweep green). Prior small ships: iter-146/147/148.)
