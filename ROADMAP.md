...
- `SHIPS_SINCE_PERSONSIM: 1`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. (=1: iter-153 shipped E9 cohesion-at-output — its MACHINERY is systems (admissions composition), not core person-sim, even though the named-arrival surface gave it SOUL 4. **iter 154 IS HARD-LOCKED back to a CORE person-sim pick** — the counter self-corrects the systems detour, keeping it people-FIRST.)
- `SMALL_SHIPS_SINCE_EPIC: 0`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC SHIPS. (=0: **EPIC SHIPPED iter 153 — E9 cohesion-at-output** (reputation→applicant tilt + named cohesion note; bounded ±5%, sweep 5✓/0 breakage). Prior small ships: iter-150/151/152.)
