...
- `SHIPS_SINCE_PERSONSIM: 0`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. (=0: iter-135 protégé follow-journal — attachment + transformation-legibility, Compass 1/2.)
- `SMALL_SHIPS_SINCE_EPIC: 2`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC SHIPS (the deploy is NOT a ship → doesn't increment). (=2: iter-137 tuition + iter-138 mood-emote. Last epic = iter-136 E8. **After the iter-139 deploy, the next polish hits 3 → an epic is owed (feature-epic OK; EPICS_SINCE_STRUCTURE=1).**)
- `EPICS_SINCE_STRUCTURE: 1`    → ≥2 ⇒ next epic must be STRUCTURE. (=1: iter-136 E8 FEATURE-epic. One more feature-epic, then structure (ui.js shared-UI-context, high-coupling). Prior structure: iter-134 data/content split.) (=0: **RESET by iter-134 STRUCTURE** — CONTENT (414 ln copy) carved data.js→`js/content.js`, byte-identical (gate green, sweep 5✓, bot BOTOK identical metrics). Took the CLEAN config/copy leaf instead of review-escaping the high-coupling ui.js. Feature-epics legal again. **Next structure candidate: ui.js (1807 ln) → the shared-UI-context refactor — high-coupling, queued for when a UI feature (E11/E12-14, owner-gated) needs it; review-escape if it comes due while still non-urgent.** Prior: iter-127 person.js carve.)
...
- **E8 ✅ CHECKPOINT 1 SHIPPED (iter 136) — Reputation-shaped teacher POOL.** Teachers are DRAWN by standing (`req:{m,v}` on each pool teacher — TT famous / UT trusted / TC substantive); locked ones show as visible aspiration ("cần Tiếng Tăm ≥ 40"), the self-taught coder always available (weak-start not doomed); +2 teachers (pool 4→6). Soft gate, manual hire still legal, sweep-neutral (UI/availability only). Connects standing→faculty→growth-pipeline (the owner's "systems separate" + "automatic based on ranking"). **NEXT (checkpoint 2, owner-playtest):** grain-flavored teacher TRADE-OFFS (a prestige engineer realizes coders, neglects makers) — a `teacherFactor` grain-specific change = a real balance change (sweep uses teacherFactor); + optionally the "1-2 quarterly applicant cards" UX. Deferred as the balance/feel half.
