...
- `SHIPS_SINCE_PERSONSIM: 0`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. The "graphics WINS" counter, repointed. (=0: iter-116 craft mismatch-adrift ceiling — a person-sim balance ship.)
- `SMALL_SHIPS_SINCE_EPIC: 2`   → ≥3 ⇒ EPIC-LOCKED. (+1 iter 115 clock, +1 iter 116 ceiling → one more polish forces the EPIC track.)
