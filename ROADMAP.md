...
- `SHIPS_SINCE_PERSONSIM: 0`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. (=0: iter-154 the wasted-gift reading at the graduation results screen — biography / legibility-of-transformation, Compass 1/2 (VISION §114). The systems detour (E9) is corrected.)
- `SMALL_SHIPS_SINCE_EPIC: 1`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC SHIPS. (=1: iter-154 grad-results realization reading (person-sim polish + DRY realCreditSuffix). Last epic = iter-153 E9.)
