...
- `SMALL_SHIPS_SINCE_EPIC: 1`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC actually SHIPS. (=1: iter-130 match-legibility polish. Last epic = iter-128 E-UNDERDOG; iter-129 was the deploy.)
- `EPICS_SINCE_STRUCTURE: 1`    → ≥2 ⇒ next epic must be STRUCTURE. (=1: iter-128 E-UNDERDOG was a FEATURE-epic. iter-127 was the structure reset. One more feature-epic allowed, then structure. Next structure candidate: ui.js (1781 ln, the painful file) → screens.js — high-coupling, needs the shared-UI-context refactor.)
