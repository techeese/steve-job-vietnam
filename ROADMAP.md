...
- `SMALL_SHIPS_SINCE_EPIC: 1`   → ≥3 ⇒ epic track locks (→ resolve via review-defer; next true epic AWAITS OWNER STEER, per iter-92). **Plateau mode**: ship genuine safe value (completeness/content/bugfix/juice/maintenance) until steer. (iter 98 = POLISH: alumLines doubled 2→4 per state.)
