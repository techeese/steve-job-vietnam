...
- `SMALL_SHIPS_SINCE_EPIC: 0`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC actually SHIPS. (=0: iter-128 shipped **EPIC E-UNDERDOG**. Prior: iter-126 E5, iter-127 STRUCTURE.)
...
- `ITERS_SINCE_DEPLOY: 9`       → **(owner 2026-06-14: "push to github.io after every 10 iteration") ≥10 ⇒ DEPLOY (run the banner recipe), then reset 0.** +1 per loop iteration. (=9: +iter-128. **DEPLOY DUE NEXT FIRING (=10).** The owner's "push after every 10 iteration" was a DIRECT standing authorization (iter-117) — attempt the push next firing; if the classifier blocks, surface it for a fresh owner go. Big backlog since the iter-117 deploy: E4/E4.1/protégé-arc/E5/structure/E-UNDERDOG.)
