...
- `SMALL_SHIPS_SINCE_EPIC: 2`   → ≥3 ⇒ the next firing is **HARD-LOCKED to the EPIC track** — which AWAITS OWNER STEER (per iter 92; resolve via review-that-defers if it fires with no worthwhile autonomous epic). (93 ticker-idle + 94 surface-hidden-ticker-lines = 2 POLISH, plateau-mode safe value.)
