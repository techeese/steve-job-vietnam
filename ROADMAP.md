...
- `SHIPS_SINCE_PERSONSIM: 0`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ HARD-LOCKS to a person-sim pick/epic.** Reset 0 only on a person-sim ship. (=0: **iter-193 PERSON-SIM ship — cohortBeat lines made EMERGENT from each kid's GIFT (tell)**: the in-play wilt/bloom glimpse now names the SPECIFIC talent being realized/wasted (spark→"đang thành kỹ sư thực thụ"/wilt "lớp này chỉ dạy chép"; sky→"làm ra thứ của riêng mình"/"chẳng có gì để chế tạo"; hype→"biết bán ý mình mà vẫn là ý thật"/"học cách lùa hơn cách làm"), serving the owner's "emergent per-life narrative, NOT generic strings" mandate. Narration-only → byte-identical bot/sweep. The needed correction after the 5-economy-ship run flagged below. Prior person-sim: iter-179 cohortBeat. The economy steer (iters 180-184) PREEMPTED the lock for 5 ships — now corrected back to the SOUL.)
- `SMALL_SHIPS_SINCE_EPIC: 2`   → **≥3 ⇒ EPIC owed; the next firing HARD-LOCKS to the EPIC track.** Reset 0 only when an EPIC SHIPS. (=2: iter-193 cohortBeat emergent-narrative (person-sim polish). +iter-184 lab-livestream income. **⚠ NEXT firing ≥3 ⇒ EPIC track owed.** Epic that reset this: **iter-180-182 owner LIVING-ECONOMY epic** (ckpts 1-3). iter-183 structure was the owed paydown.)
