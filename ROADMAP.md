...
- `SMALL_SHIPS_SINCE_EPIC: 1`   → **≥3 ⇒ EPIC owed; the next firing HARD-LOCKS to the EPIC track.** Reset 0 only when an EPIC SHIPS. (=1: iter-174 small person-sim ship — activated the dead `channeledMaker` §C-4 beat (a named face for the balanced school's quiet channeling), completing the iter-172 feature. Epic that reset this: **iter-172 [EVOLUTION] Three Theses, Three Epilogues** (closed the §D-3 gap; reset the 168–171 small-beat run). iter-173 was an incident HOTFIX [debt-valve, exempt]. At 1 — 2 more small ships before the next firing is epic-locked.)
