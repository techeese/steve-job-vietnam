...
- `SHIPS_SINCE_PERSONSIM: 0`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. The "graphics WINS" counter, repointed. (=0: iter-124 E4.1 — realization poles cleanup + the mentor's hand named — is a person-sim ship.)
- `SMALL_SHIPS_SINCE_EPIC: 1`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC actually SHIPS. (=1: iter-124 E4.1 person-sim polish. Last epic = iter-123 E4. The iter-120/121 maturity-plateau "epic deferred" hold is OVER — "decide→ship→veto-by-reaction" held.)
...
- `ITERS_SINCE_DEPLOY: 5`       → **(owner 2026-06-14: "push to github.io after every 10 iteration") ≥10 ⇒ DEPLOY (run the banner recipe), then reset 0.** +1 per loop iteration. (=5: +iter-124; first deploy was iter-117 → next deploy ≈ iter 127.) Note: the public push needs a DIRECT owner go OR a `Bash(git push origin main)` allow-rule (the classifier blocks a feedback.md-only authorization).
