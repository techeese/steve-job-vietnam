...
- `SMALL_SHIPS_SINCE_EPIC: 1`   → ≥3 ⇒ epic track locks (→ resolve via review-defer; epic AWAITS OWNER STEER, per iter-92). (iter 102 = POLISH: PWA install.) **Plateau mode**: content/dead-content/coverage veins exhausted AND distribution (OG iter-101 + PWA iter-102) now done too — remaining safe value is essentially gone; lean toward periodic maintenance + awaiting steer over micro-churn.
