...
- `SHIPS_SINCE_PERSONSIM: 1`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. The "graphics WINS" counter, repointed. (=1: the owner-directed Art & Polish epic shipped since the Mentor's Ledger soul-loop → the loop returns to people-first next firing.)
- `SMALL_SHIPS_SINCE_EPIC: 0`   → ≥3 ⇒ epic locks. (Reset by the Art epic ship.)
