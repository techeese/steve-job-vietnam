...
- `SHIPS_SINCE_PERSONSIM: 0`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. The "graphics WINS" counter, repointed. (=0: iter-127 was a behavior-neutral STRUCTURE carve of the person-sim itself — person-sim-domain, not drift; stays 0. Prior person-sim ship: iter-126 E5.)
- `SMALL_SHIPS_SINCE_EPIC: 0`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC actually SHIPS. (=0: iter-126 shipped **EPIC E5** — discoverable talent. Prior polish: iter-124 E4.1, iter-125 protégé-arc.)
- `EPICS_SINCE_STRUCTURE: 0`    → ≥2 ⇒ next epic must be STRUCTURE. (=0: **RESET by iter-127 STRUCTURE** — the person destiny/realization/protégé layer (cascade/makeAlumnus/flourishOf/realFrac/realClass/favBeat) carved engine.js→`js/sim/person.js`, proven byte-identical (sweep diff empty, gate green, bot BOTOK). engine 1203→1104. De-risks E6/E-UNDERDOG. Feature-epics legal again. Next structure candidate when due: ui.js (1781 ln, the painful file) → screens.js — but high-coupling, needs the shared-UI-context refactor. Prior: iter-114 person.js carve.)
