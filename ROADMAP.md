...
- `SHIPS_SINCE_PERSONSIM: 0`    → **(PEOPLE-FIRST ARC) ≥1 ⇒ this firing HARD-LOCKS to a person-sim pick/epic (Compass 1/2); a non-person-sim pick is ILLEGAL.** Reset 0 only on a person-sim ship. (=0: iter-150 protégé graduation beat carries the realization coda — the watchable-arc culmination, Compass 1/2 (VISION §114).)
- `SMALL_SHIPS_SINCE_EPIC: 1`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC SHIPS. (=1: iter-150 protégé-coda polish. Last epic = iter-149 STRUCTURE epilogue.js. **50-FIRING REVIEW (iter 150): safe person-sim space is MATURE; recent small ships are correct (highest-value-safe on a release-ready build); real unlock = owner re-engagement, surfaced not forced — see CHANGELOG iter-150 reflection.**)
