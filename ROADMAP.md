...
- `SMALL_SHIPS_SINCE_EPIC: 0`   → **≥3 ⇒ EPIC owed.** Reset 0 only when an EPIC SHIPS. (=0: iter-140 shipped **E7p (prizes)** — a feature-epic. Prior polish: iter-137 tuition, iter-138 mood-emote.)
- `EPICS_SINCE_STRUCTURE: 2`    → **≥2 ⇒ the NEXT epic MUST be a STRUCTURE move.** (=2: iter-136 E8 + iter-140 E7p, both FEATURE-epics. **Next epic = STRUCTURE** = the ui.js shared-UI-context refactor (high-coupling, the only remaining structure) OR a rigorous review-escape (it's non-urgent: future UI features E11/E12-14 are owner-gated). Prior structure: iter-134 data/content split.) (=0: **RESET by iter-134 STRUCTURE** — CONTENT (414 ln copy) carved data.js→`js/content.js`, byte-identical (gate green, sweep 5✓, bot BOTOK identical metrics). Took the CLEAN config/copy leaf instead of review-escaping the high-coupling ui.js. Feature-epics legal again. **Next structure candidate: ui.js (1807 ln) → the shared-UI-context refactor — high-coupling, queued for when a UI feature (E11/E12-14, owner-gated) needs it; review-escape if it comes due while still non-urgent.** Prior: iter-127 person.js carve.)
