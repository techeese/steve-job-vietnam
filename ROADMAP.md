...
- `ITERS_SINCE_DEPLOY: 2`       → **(owner 2026-06-14: "push to github.io after every 10 iteration") ≥10 ⇒ DEPLOY (run the banner recipe), then reset 0.** +1 per loop iteration. (=2: +iter-141 (maintenance, no code → nothing new to deploy). **DEPLOYED iter 139 — LIVE & VERIFIED** `main 40916f7..559adaf` pushed; live serves `?v=1781467721271`. 10 commits now public: match-legibility · mood+burnout+warning · protégé arc/journal/capstone · E8 faculty · tuition trade-off · content.js split · mood-emote. Pre-deploy gate GREEN, bot BOTOK, 5 sweep ✓ / 0 breakage; standing authorization held. Next deploy ≈ iter 149.)
