...
| **National layer (value-chain)** | CP1–3 built behind OFF flag (owner-gated flip) | strong talent, but the SYSTEM thins the top tail — value EXTRACTED offshore (the FDI assembly trap), the late-stage CAPITAL CLIFF (a real founder can't scale), BRAIN DRAIN (the best emigrate). The policy-plan sibling's sharpest gap. | **iter-271 (Phase-3 "Giá trị ở lại"):** `a.standing` RETAINED/EXTRACTED/DRAINED on a realized alumnus (flourish≥4), set once from a SEPARATE seed0-salted generator → **NON-APEX** (🍎 gate unchanged; GATE_STANDING firewall ON===OFF); structural pulls (era/archetype/origin + brain-drain flag) heavy, player levers (mentor + `renDial`) light + clamped (non-dominant); capstone prose-key "Giá trị ở lại?" reading. `STANDING.ON=false` default → byte-identical; owner flips to go live (CP4 + the flip owner-gated) | era, archetype, the formation levers, the alumni FSM, the value-for-dân capstone axis |
