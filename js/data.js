...
var CONFIG = {
  ...
  WRONGMAJOR_BEAT_GAP: 120, // iter-270 (Phase-2c CP3b): the wrong-major wilt beat — when an OFF-native grain (open-door cram) is nguội-ing in a khoa that doesn't fit, name the "right gift, wrong major" waste. Once-a-season-ish; only fires when off-native grains exist (open-door) → byte-identical under the default "native" policy.
  FULL_BEAT_GAP: 130, // iter-256 (scout #3): the "out of hands" beat — when ALL mentor slots are spent AND a strong gift is wilting unrescued, name the cost of the full ledger (the tragic-allocation, proactively). Rarer than cohortBeat so it stays a weighty moment. Only fires once you've filled the ledger → headless (no mentoring) never triggers it → byte-identical.
  ...
  INTAKE_META: { // iter-268 (Phase-2c CP3) — the intake-rule dial labels + open-question tradeoffs (each choice realizes some, wastes some — no dominant rule)
    ...
    open:   { label: "Mở cửa",   tradeoff: "không đứa nào ngồi không — nhưng đứa lệch khoa phải học cái nghề chẳng hợp tạng, dễ nguội." }
  },
