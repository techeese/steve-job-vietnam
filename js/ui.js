...
(function () {
  ...
  // NB: buildCast / cardLife / shareCard moved to js/epilogue.js (iter 149 STRUCTURE) — the pure, stateless
  // slice of the epilogue cluster (params + globals only). essayDraft (below) calls them by bare name (they're
  // globals now). saveShareCard stays here (it needs the closure-private toast()); essayDraft stays (S/tpl/etc.).
  ...
  function essayDraft(capstone) {
    ...
    if (s.META.graduated === 0) {
      ...
      if (proteges[0]) {
        ...
        var coda = CONTENT.protegeCoda[protegeCodaKey(pg.state, pseed)]; // iter-150: ONE source of truth, shared with the graduation beat (engine.js)
