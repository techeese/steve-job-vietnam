...
(function () {
  ...
  function boot() {
    ...
    var aq = location.search.match(/[?&]arch=(\w+)/);
    if (aq && CONFIG.ARCHETYPES[aq[1]] && (!localStorage.getItem(CONFIG.SAVE_KEY))) {
  ...
  function showInspectStudent(id) {
    ...
    ins.innerHTML =
      ...
      "<button class='ix' id='mentorBtn' title='Dìu dắt — dồn tâm sức (tối đa " + (HVS.mentorSlots ? HVS.mentorSlots() : CONFIG.MENTOR_SLOTS) + ")'>" + (st.mentored ? "🎓" : "➕") + "</button>" +
      ...
      "<div class='iflav'>Tiềm năng (hạt giống): " + tr.txt + (tr.lvl < 2 ? " <span class='tiny' style='color:var(--faint)'>(dìu dắt để biết rõ)</span>" : "") + " &nbsp;·&nbsp; Tâm sức dìu dắt: " + (HVS.mentorCount ? HVS.mentorCount() : 0) + "/" + (HVS.mentorSlots ? HVS.mentorSlots() : CONFIG.MENTOR_SLOTS) + "</div>" +
    ...
    $("mentorBtn").onclick = function () {
      ...
      if (!r.ok) { if (r.why === "full") toast("Hết suất dìu dắt — bạn chỉ dồn tâm sức cho " + (HVS.mentorSlots ? HVS.mentorSlots() : CONFIG.MENTOR_SLOTS) + " em một lúc. Phải chọn."); return; }
  ...
  // NB: the epilogue ESSAY (buildEssay + numWord/isOldCohort), the cast (buildCast) and the end-card
  // (shareCard) all live in js/epilogue.js now — pure over the loaded state + globals (iter 149/183
  // STRUCTURE). essayDraft below is a thin wrapper passing S() + the two ui.js-private callbacks
  // (saveShareCard / hideModal). saveShareCard stays here (it needs the closure-private toast()).
  ...
  function essayDraft(capstone) { return buildEssay(S(), { save: saveShareCard, fold: hideModal }, capstone); } // iter-183 STRUCTURE: body carved to js/epilogue.js buildEssay() (byte-verified via lives.sh)
